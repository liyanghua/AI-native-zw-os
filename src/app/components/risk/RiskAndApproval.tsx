...
export function RiskAndApproval() {
  ...
  const query = repositories.riskApproval.getOverview();
  ...
  return (
    <div className="p-8 space-y-6">
      ...
      <section className="grid grid-cols-3 gap-4">
        <SummaryCard label="风险事件" value={viewModel.summary.exceptionCount} />
        <SummaryCard label="高风险待审批" value={viewModel.summary.highRiskApprovals} />
        <SummaryCard label="低置信度决策" value={viewModel.summary.lowConfidenceDecisions} />
      </section>
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-6">
        ...
        <div className="space-y-3">
          {viewModel.exceptions.map((exception) => (
            <div key={exception.id} className="rounded-xl border border-orange-200 bg-orange-50 p-4">
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  ...
                  <div className="mt-1 text-sm text-slate-600">{exception.severityLabel}</div>
                  {exception.projectId && (
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-6">
        ...
        <div className="space-y-3">
          {viewModel.pendingApprovals.map((action) => (
              ...
              <div className="flex items-start justify-between gap-4">
                <div className="space-y-2">
                  ...
                  <div className="text-xs text-slate-500">
                    {action.riskLabel} · 责任人 {action.owner} · {action.writebackStatusLabel}
                  </div>
                  <Link to={`/project/${action.projectId}`} className="text-sm text-blue-600 hover:text-blue-700">
                    查看项目
                  </Link>
                </div>
                <div className="flex gap-2">
                  <button
                    onClick={() => actions.rejectAction(action.id, "当前风险过高，暂不执行")}
      ...
      <div className="grid grid-cols-[0.8fr,1.2fr] gap-6">
        <section className="rounded-2xl border border-slate-200 bg-white p-6">
          ...
          <div className="space-y-3">
            {viewModel.lowConfidenceProjects.map((project) => (
              <div key={project.id} className="rounded-xl bg-slate-50 p-4">
                <div className="flex items-start gap-3">
                  <Brain className="mt-0.5 size-4 text-violet-600" />
                  <div className="space-y-1">
                    <div className="font-medium text-slate-900">{project.name}</div>
                    <div className="text-sm text-slate-600">{project.problemOrOpportunity}</div>
                    <div className="text-xs text-slate-500">当前置信度：{project.confidence}</div>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <div className="mt-1 text-sm text-slate-600">
                      {policy.decisionModeLabel} · {policy.riskLabel}
                    </div>
